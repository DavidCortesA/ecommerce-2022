...
const Sidebar = () => {
  ...
  return (
    <div className={`${isOpen ? 'right-0': '-right-full'} w-full bg-white fixed top-0 h-full shadow-2xl md:w-[35vw] transition-all duration-300 z-20 px-4 lg:px-[35px]`}>
      ...
      <div className='flex flex-col gap-y-2 h-[250px] lg:h-[300px] overflow-y-auto overflow-x-hidden'>{cart.map(item => {
      ...
      <div className=' flex flex-col gap-y-3 py-4 mt-4'>
        ...
        <Link to='/ecommerce-2022' className='bg-gray-200 flex p-4 justify-center items-center text-primary w-full font-medium'>View cart</Link>
        <Link to='/ecommerce-2022' className='bg-primary flex p-4 justify-center items-center text-white w-full font-medium'>Checkout</Link>
      </div>
    </div>
  );
};
